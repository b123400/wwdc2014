...
-(void)zoomInShow:(UIView*)theView{
    ...
    if(UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPhone){
        evaluator=[[SecondOrderResponseEvaluator alloc] initWithOmega:14.825 zeta:0.44];
	}else{
        evaluator=[[SecondOrderResponseEvaluator alloc] initWithOmega:14.825 zeta:0.54];
    }
...
-(NSArray*)popToViewController:(UIViewController *)viewController animated:(BOOL)animated{
	if([[self viewControllers]count]<=2)return [NSArray arrayWithObject:[self topViewController]];
	if(viewController){
		nextController=viewController;
	}else{
